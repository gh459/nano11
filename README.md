...
- **🛡️ Customization Options (Issues #1, #9, #10, #12, #13)**:
  ...
  - **🔄 OOBE Boot Loop & Setup Crash Fixed**: Solved the infinite boot loop at the "Please wait" ("お待ちください") screen. Root causes fully resolved:
    ...
    - **Reboot Loop Trap Removed (`ErrorHandler.cmd`)**: Completely eliminated `ErrorHandler.cmd` from the unattended answer file, which previously issued `shutdown /r /t 1` on any setup error, causing an infinite reboot loop.
